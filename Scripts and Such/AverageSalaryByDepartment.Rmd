...
```{r}
plot <- (ggplot(top10_avgsal, aes(x = reorder(Department, avg_sal), y = avg_sal, fill = "red2")) + geom_bar(stat = "identity") + coord_flip()) +labs(title = "Greatest Average Salary by Department", x = "Salary (in USD)", y = "Department") + ylim(0, 160000)

plot <- (ggplot(top10_avgsal, aes(x = reorder(Department, avg_sal), y = avg_sal, fill = Department)) + geom_bar(stat = "identity") + coord_flip())
plot 

plot <- (ggplot(top10_avgsal, aes(x = reorder(Department, avg_sal), y = avg_sal, fill = "red2")) + geom_bar(stat = "identity") + coord_flip()) +labs(title = "Average Salary by Department", x = "Salary (in USD)", y = "Department")
...
```{r}
boxSalaries = unoSalaryByDept[which((unoSalaryByDept$Department == "Economics")|(unoSalaryByDept$Department == "Mathematics")|(unoSalaryByDept$Department == "School of Accounting")|(unoSalaryByDept$Department == "School of Accounting")|(unoSalaryByDept$Department == "Dept of Finance Banking & Real Est")|(unoSalaryByDept$Department == "Computer Science")|(unoSalaryByDept$Department == "ISQA")|(unoSalaryByDept$Department == "Psychology") | (unoSalaryByDept$Department == "Biomechanics") | (unoSalaryByDept$Department == "College of Info Science and Tech") | (unoSalaryByDept$Department == "Marketing & Entrepreneurship")|(unoSalaryByDept$Department == "Student Success")| (unoSalaryByDept$Department == "Management")|(unoSalaryByDept$Department == "School of Interdisciplinary Information")|(unoSalaryByDept$Department == "Academic Affairs")|(unoSalaryByDept$Department == "School of Criminology & Crim Justice")),] %>% group_by(Employee) %>% summarise(Budgeted..Annual.Salary = sum(Budgeted..Annual.Salary), Department = Department) %>% distinct()

# Box-plots of mpg by number of gears
# observations (points) are overlayed and jittered

boxSalaries %>% 
  ggplot(aes(y = Budgeted..Annual.Salary, x = factor(Department, levels=c("Dept of Finance Banking & Real Est","School of Accounting","Management","Economics","ISQA","Computer Science","School of Interdisciplinary Information","Academic Affairs","Student Success","Marketing & Entrepreneurship","College of Info Science and Tech","School of Criminology & Crim Justice","Biomechanics","Psychology","Mathematics")))) + 
  geom_boxplot(fill = "azure2") + 
  labs(title = "Salary Spread by Department (Red: Mean)", x = "Department", y = "Salary($)") + coord_flip() + stat_summary(fun = mean, geom = "point", col = "brown2") + theme_light() + scale_y_continuous(labels = comma)
