...
```{r}
plot <- (ggplot(top10_avgsalUNL, aes(x = reorder(Department, avg_sal), y = avg_sal, fill = "red")) + geom_bar(stat = "identity") + coord_flip()) +labs(title = "Greatest Average Salary by Department for UNL", x = "Salary (in USD)", y = "Department") + ylim(0, 200000)
...
unldepartmentData <- unlSalaryByDeptWithAvg[order(unlSalaryByDeptWithAvg$avg_sal),]

trimmedunlData <- unlSalary[,c(1:5)] 

boxSalaries = trimmedunlData[which((unlSalary$Department == "School of Accountancy")|(unlSalary$Department == "Finance")|(unlSalary$Department == "Supply Chain Management & Analytics")|(unlSalary$Department == "Management")|(unlSalary$Department == "Marketing")|(unlSalary$Department == "Athletics")|(unlSalary$Department == "Economics")|(unlSalary$Department == "Electrical & Computer Engineering") | (unlSalary$Department == "Chemical & Biomolecular Engineering") | (unlSalary$Department == "College of Info Science and Tech") | (unlSalary$Department == "Marketing & Entrepreneurship")|(unlSalary$Department == "Office of Vice Pres/Vice Chancellor")| (unlSalary$Department == "Computer Science & Engineering")|(unlSalary$Department == "J S Raikes School of Comp Sci & Mgmt")|(unlSalary$Department == "College of Law")|(unlSalary$Department == "Mechanical & Materials Engineering")),] %>% group_by(Employee) %>% summarise(Budgeted..Annual.Salary = sum(Budgeted..Annual.Salary), Department = Department) %>% distinct()
...
boxSalaries %>% 
  ...
  geom_boxplot() + ylim(0,300000) + labs(title = "Salary Spread by Department", x = "Salary", y = "Department") + coord_flip() 
